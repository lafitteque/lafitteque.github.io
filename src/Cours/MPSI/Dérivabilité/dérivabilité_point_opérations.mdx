...
<Item type='definition' titre='dérivabilité en un point' searchDesc="fonction dérivée">
...
<Item type='definition' titre='tangente'  searchDesc="droite ">
...
<Item type='definition' titre='dérivabilité à gauche / droite' searchDesc="fonction">
...
<Item type='proposition' titre='caractérisation de la dérivabilité'  searchDesc="en a d'une fonction">

$f$ est dérivable en $a$ si et seulement si il existe $C \in \R$ et une fonction $\varepsilon$ définie sur $J\setminus \{ 0\} $ (où $J$ est un intervalle ouvert contenant $0$) et telle que $\limm{\substack{x \to 0 \\ x \neq 0}} \varepsilon(h) = 0$ et : $\forall x \in J\setminus \{ 0\} \v f(a + h) = f(a) + C.h + h.\varepsilon(h)$
...
$\cd$ Montrons d'abord le sens réciproque : on a $f(a + h) = f(a) + C.h + h.\varepsilon(h)$ puis $\dfrac{f(a + h) - f(a)}{h} = C + \varepsilon(h) \limto{h \to 0} C$ d'où la dérivabilité de $f$ en $a$

$\cd$ Supposons que $f$ est dérivable en $a$. On pose $C = f'(a)$ et $\varepsilon$ telle que $\varepsilon(h) = \dfrac{f(a + h) - f(a)}{h} - C$ définie sur $J \setminus \{ 0\}$ où $J$ est un intervalle ouvert contenant $0$ (il suffit d'en prendre un dans $I$).

</Hideable>

<Item type='proposition' titre='dérivabilité et opérations' searchDesc="fonction">
...
<Item type='proposition' titre='dérivabilité et continuité'  searchDesc="fonction">
...
Comme $f$ est dérivable en $a$, il existe une fonction $\varepsilon$ définie sur $J \setminus \{0\}$ où $J$ est un ouvert contenant $0$ telle que : $f(a + h) = f(a) + h.f'(a) + h.\varepsilon(h)$. 

En effectuant le changement de variable $x = a+h$, on a $f(x) = f(a) + (x-a).f'(a) + (x-a). \varepsilon(x-a) \limto{x \to a} f(a)$ d'où la continuité de $f$ en $a$.
</Hideable>

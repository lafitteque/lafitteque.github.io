...
Soit $\epsilon > 0$, il existe $(N_1\v N_2) \in \N^2$ convenant pour la définition de la convergence pour $\suitedefN{u_{2n}}$ et $\suitedefN{u_{2n+1}}$.

Si $n \geqslant N = max(N_1 \v N_2)$, alors 

$$
|u_{2n} - l|\leqslant \epsilon \ET|u_{2n+1} - l|\leqslant \epsilon \quad (1)
$$ 

Soit $n \in \N$, il existe $k \in \N$ tel que $n = 2k$ ou $n = 2k+1$.


Mais donc, si $n \geqslant 2N$, $k \geqslant \dfrac{n}{2} \geqslant N$. On applique donc $(1)$ à $k$ et on pose $\tilde{N} = 2N$ pour obtenir :

$$
\forall \epsilon > 0 \v \exists \tilde{N} \in \N \mid n \geqslant \tilde{N} \Rightarrow |u_n - l| \leqslant \epsilon
$$
</Hideable>

...
$\cd$ le zéro est la matrice $0_{\cM_n(\R)} = \begin{pmatrix} 0 & \dotsm & 0 \\ \vdots & \ddots & \vdots \\ 0 & \dotsm & 0\end{pmatrix}$.
...
$E_{i,j} . A \v T_{i,j}(\lambda). A$ et $\tau_{i,j} . A$ sont toutes les trois inversibles.
